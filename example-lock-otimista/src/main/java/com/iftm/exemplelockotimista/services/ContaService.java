...
public class ContaService {
    ...
    public ResponseEntity<?> sacar(String numConta, Double valor) {
        var conta = contaRepository.findByConta(numConta).orElseThrow(() -> new ResponseStatusException(BAD_REQUEST, "Conta não encontrada"));
    ...
    public ResponseEntity<?> deposito(String numConta, Double valor) {
        ...
        if (!conta.depositar(valor))
            return ResponseEntity.status(400).body("O valor digitado é incorreto");
