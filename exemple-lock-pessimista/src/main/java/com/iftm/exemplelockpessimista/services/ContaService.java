...
public class ContaService {
    ...
    public ResponseEntity<?> sacar(String conta, Double valor) {
        var contaDb = contaRepository.findByConta(conta).orElseThrow(() -> new ResponseStatusException(BAD_REQUEST, "A conta informada não foi encontrada"));
    ...
    public ResponseEntity<?> depositar(String conta, Double valor) {
        ...
        if (!contaDb.depositar(valor))
            return ResponseEntity.status(400).body("O valor informado não pode ser depositado");
